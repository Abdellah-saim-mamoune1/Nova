...
export function GetHelp() {
  ...
  const handleConfirm = async () => {
    ...
    if (AccountId != null) {
     ...
     const response= await AddNewGetHelpRequist(requist);
  ...
  return (
    <div className="flex  flex-1 flex-col items-center justify-center h-full">
      ...
      {notificationVisible && (
        ...
        ${Status==="Succeeded"?"bg-teal-600":"bg-red-600"} text-white px-6 py-4 rounded-xl shadow-2xl 
         z-[9999] transition-all ease-in-out animate-slide-down
          flex items-start gap-3 max-w-[90%] sm:max-w-md`}>
          <div>
            <p className="font-semibold">{Status==="Succeeded"?"Your request has been sent successfully!":
